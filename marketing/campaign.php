...
<body>
				...
				<div class="container-fluid" style="height: auto;">
					<div class="panel panel-default">
                                        ...
                                        <div style="margin: 10px; margin-bottom: 10px; ">
                                            <button type="button" id="add" class="btn btn-default btn-xs" onclick="addCampaign()" style="float: right; margin: 10px;">Add Campaign</button>
                                        </div>
                                    <br>
                                    <div class="panel-body">
                                       <table style="margin: 10px;"class="table table-bordered table-responsive display" id="myTable" width="98.4%">
                                            <thead style="font: bolder">
                                               ...
                                               <tbody>
                                                   <?php
                                                   $result = mysqli_query($connect, "SELECT * FROM tbl_campaign");
                                                   
                                                   while($row= mysqli_fetch_assoc($result)):
                                                   ?>
                                                   <tr>
                                                       <td><a href="campaign_info.php?id='<?php echo $row['id']?>'"><?php echo $row['title']; ?></a></td>
    ...
    <script>
        function addCampaign(){
            window.open('add_campaign.php','_self',false);
        }
    </script> 
    <script>
        function addExpense(){
...
if(isset($_POST['btnEdit'])){
    ...
    $new_promo_code=mysqli_real_escape_string($con,$_POST['promocode']);
    $new_discount=mysqli_real_escape_string($con,$_POST['discount']);
    $new_startDate=mysqli_real_escape_string($con,$_POST['startdate']);
    $new_endDate=mysqli_real_escape_string($con,$_POST['enddate']);
    $new_promo_limit=mysqli_real_escape_string($con,$_POST['limit']);
    
    $sqlupdate="UPDATE tbl_campaign SET promo_code='$new_promo_code',discount='$new_discount',startDate='$new_startDate',enddate='$new_endDate',promo_limit='$new_promo_limit'
                WHERE id='$new_id'";
