...
<html>
    <head>
        <head>
	<title>Revenue | Expense Category</title>
	<meta charset="utf-8">
...
<body>
								...
								<ul class="nav">
									...
									<li>
                                                                        ...
                                                                        <div id="subPages3a" class="collapse ">
                                                                            <ul class="nav">
                                                                                <li><a href="expense_category.php" class="active">Expense Category</a></li>
				...
				<div class="container-fluid" style="height: auto;">
					<div class="panel panel-default">
                                        ...
                                        <div style="margin: 10px; margin-bottom: 10px; ">
                                            <button type="button" id="add" class="btn btn-default btn-xs" onclick="addExpenseCat()" style="float: right; margin: 10px;">Add</button>
                                        </div>
                                    <br>
                                       ...
                                       <table style="margin: 10px;"class="table table-bordered table-responsive display" id="myTable" width="98.4%">
                                            <thead style="font: bolder">
                                                <tr>
                                                 ...
                                                 <tbody>
                                                   <?php
                                                   $result = mysqli_query($connect, "SELECT * FROM expense_category");
                                        ...
                                        <input type="hidden" id="table" name="table" value='expense_category'>
                                        <input type="hidden" id="returnto" name="returnto" value='../revenue/expense_category.php'>
    ...
    <script>
        function addExpenseCat(){
            window.open('add_expensecat.php','_self',false);
        }
    </script> 
    ...
    <script>
        ...
        $(document).on('change','#dropdown',function(e){
            ...
            var table = "expense_category";

            $.post('../fleet_tracking/update.php',{dropId:id, dropVal:val, dropTbl:table},function(data){
                alert(data);
            });
        });
    </script>
...
if(isset($_POST['btnEdit'])){
    ...
    $new_name=mysqli_real_escape_string($con,$_POST['expcat']);
    
    $sqlupdate="UPDATE expense_category SET status='$new_status',category='$new_name'
                WHERE id='$new_id'";
    $result_update=mysqli_query($con,$sqlupdate);
    if($result_update){
        echo '<script>window.location.href="expense_category.php"</script>';  
    }
    else{
...
if(isset($_POST['btnDel'])){
    ...
    $new_id=mysqli_real_escape_string($con,$_POST['txtid']);
    $sql_del = "DELETE FROM expense_category WHERE id='$new_id'";
    $runDel = mysqli_query($connect, $sql_del);
    if($runDel)
    {
        echo '<script>alert("Expense Category Deleted")</script>';
    }
    else{
        echo '<script>alert("Update Failed")</script>';
    }
