...
<html>
    <head>
        <head>
	<title>Fleet Tracking | Manage Drivers</title>
	<meta charset="utf-8">
...
<body>
                                ...
                                <div class="">
                                    <div class="panel panel-default">
                                        <div class="panel-heading">
                                            <h3 class="panel-title">Manage Drivers</h3>
                                        </div>
                                    <br>
                                    <form action="showChecked.php" method="post">
                                       <table style="margin: 10px;"class="table table-striped table-bordered" id="myTable" width="98.4%">
                                            <thead style="font: bolder">
                                               ...
                                               <tbody>
                                                   <?php
                                                   $result = mysqli_query($connect, "SELECT tbl_driver.id AS id, tbl_driver.fleet_co AS company_id, tbl_driver.status AS status, tbl_driver.firstname AS firstname, tbl_driver.lastname AS lastname, tbl_driver.email AS email, tbl_driver.mobile AS mobile, tbl_driver.license_no AS license_no, tbl_driver.photo AS photo, companies.company_name AS company_name FROM tbl_driver INNER JOIN companies ON tbl_driver.fleet_co=companies.id WHERE tbl_driver.id <>'10'");
                                                   ...
                                                   <tr>
                                                       ...
                                                       <td><a href="driver_info.php?id='<?php echo $row['id']; ?>'"><?php echo $row['firstname']." ".$row['lastname']; ?></a></td>
                                                       <td><?php echo $row['email']; ?></td>
                                                       <td><a href="company_info.php?id='<?php echo $row['company_id']; ?>'"><?php echo $row['company_name']; ?></a></td>
                                                       <td><?php echo $row['mobile']; ?></td>
                                                       <td><?php echo $row['license_no']; ?></td>
                                                       <td><img src="img/drivers/photo/<?php echo $row['email'].$row['photo']?>" class="img-thumbnail" style="width:90px; height: 85px;"/></td>
                                                       <td>IN</td>
                                                       <td><button type="button" id="getEdit" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal" data-id="<?php echo $row['id']?>"><i class="fa fa-edit" title="Edit"></i></button><button type="button" id="getWallet" style = "margin-left:1px;" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal3" data-id="<?php echo $row['id']?>"><i class="fa fa-money" title="Wallet"></i></button></td>
                                                   </tr>
                                                   <?php endwhile; ?>
                                               </tbody>
                                    ...
                                    <div style="margin: 10px; margin-bottom: 10px; ">
                                        ...
                                        <input type="hidden" id="table" name="table" value='tbl_driver'>
                                        <input type="hidden" id="returnto" name="returnto" value='manage_drivers.php'>
	...
	<script src="../assets/scripts/klorofil-common.js"></script>
        ...
        var t = $('.table').DataTable( {
           ...
           buttons: [
            ...
            { "text":'Add Driver',"className": 'btn btn-default btn-xs', action: function(){
                    addDriver();
            } }
        ]
        } );
    ...
    <script>
        $(document).on('click','#getWallet',function(e){
            e.preventDefault();
            var per_id=$(this).data('id');
            //alert(per_id);
            $('#content-data3').html('');
...
if(isset($_POST['btnEdit'])){
        ...
        move_uploaded_file($image_tmp, "img/drivers/photo/$email"."$image");
    
    $sqlupdate="UPDATE tbl_driver SET firstname='$firstname',lastname='$lastname',email='$email', gender='$gender', birthdate='$birth_date', license_expiry='$license_exp', pco_license='$pco_license', pco_expiry='$pco_expiry', insurance_no='$drivers_ins', insurance_expiry='$ins_exp', national_insNo='$national_ins', national_insExpiry='$national_insExp', mobile='$mobile_no', fleet_co='$fleet_co', address='$address',country='$country',state='$state',city='$city', photo='$image' WHERE id='$per_id'";
