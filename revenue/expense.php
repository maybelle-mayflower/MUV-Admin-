						...
						<li>
							...
							<div id="subPages3" class="collapse ">
								<ul class="nav">
									...
									<li>
                                                                        <a href="#subPages3a" data-toggle="collapse" class="active"><span>Expense Management</span><i class="icon-submenu lnr lnr-chevron-left"></i></a>
				...
				<div class="container-fluid" style="height: auto;">
					<div class="panel panel-default">
                                        ...
                                        <div style="margin: 10px; margin-bottom: 10px; ">
                                            <button type="button" id="add" class="btn btn-default btn-xs" onclick="addExpense()" style="float: right; margin: 10px;">Add</button>
                                        </div>
                                    <br>
                                    <div class="panel-body">
                                       <table style="margin: 10px;"class="table table-bordered table-responsive display" id="myTable" width="98.4%">
                                            <thead style="font: bolder">
                                               ...
                                               <tbody>
                                                   <?php
                                                   $result = mysqli_query($connect, "SELECT tbl_expense.id as id, tbl_expense.exp_management as exp_management, tbl_expense.category_id as category_id, tbl_expense.exp_date as exp_date, tbl_expense.amount as amount, expense_category.category as category_name FROM tbl_expense INNER JOIN expense_category ON tbl_expense.category_id=expense_category.id");
                                                   
                                                   while($row= mysqli_fetch_assoc($result)):
                                                   ?>
                                                   <tr>
                                                       <td></td>
                                                       <td><?php echo $row['exp_management']; ?></td>
                                                       <td><?php echo $row['category_name']; ?></td>
                                                       <td><?php echo $row['exp_date']; ?></td>
                                                       <td><?php echo $row['amount']; ?></td>
                                                       <td><button type="button" id="getEdit" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal" data-id="<?php echo $row['id'];?>"><i class="fa fa-edit" title="Edit"></i></button> <button type="button" id="getDel" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal2" data-id="<?php echo $row['id'];?>"><i class="fa fa-remove" title="delete" style="font-size:10px;color:red"></i></button></td>
                                                       </tr>
                                                   <?php endwhile; ?>
                                               </tbody>
                                        </table>
	...
	<script src="../assets/scripts/klorofil-common.js"></script>
     ...
     <script src="https://cdn.datatables.net/1.10.16/js/jquery.dataTables.min.js"></script>
        ...
        $('.table tbody input[type="checkbox"]').prop('checked', this.checked);
          ...
          <script>
        function addExpense(){
            window.open('add_expense.php','_self',false);
        }
    </script> 
...
if(isset($_POST['btnEdit'])){
    ...
    $new_man=mysqli_real_escape_string($con,$_POST['expman']);
    $new_cat=mysqli_real_escape_string($con,$_POST['expcat']);
    $new_amt=mysqli_real_escape_string($con,$_POST['expamt']);
    $new_date=mysqli_real_escape_string($con,$_POST['expdate']);
    $new_remark=mysqli_real_escape_string($con,$_POST['remark']);
    
    $sqlupdate="UPDATE tbl_expense SET exp_management='$new_man',category_id='$new_cat',amount='$new_amt', exp_date='$new_date',remarks='$new_remark'
                WHERE id='$new_id'";
...
if(isset($_POST['btnDel'])){
    ...
    $new_id=mysqli_real_escape_string($con,$_POST['txtid']);
    $sql_del = "DELETE FROM tbl_expense WHERE id='$new_id'";
    $runDel = mysqli_query($connect, $sql_del);
    if($runDel)
    {
